...
/**
 * Franchise Confirm Receipt - Accept order or report discrepancies
 */
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribe(['DISCREPANCY_VENDOR_ACKNOWLEDGED', 'DISCREPANCY_FRANCHISE_CLOSED'], (type, data) => {
      ...
      if (data.reference_id && orderDiscrepancies.discrepancies.some(d => d.id === data.reference_id)) {
        console.log('🔄 Refreshing order discrepancies due to update');
        fetchOrderAndDiscrepancies();
      }
    });
              ...
              if (totalAdjustment !== 0) {
                return (
                  <div style={{
                    ...
                    {totalAdjustment > 0 ? '+' : ''}{formatCurrency(totalAdjustment)} adjusted
                  </div>
                );
              }
              return null;
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    return (
                      ...
                      >
                        <span style={{ fontSize: 13, color: '#6b7280' }}>
                          {isOverage ? '📦' : '⚠️'} {disc.item_name} ({isOverage ? 'Overage' : 'Shortage'})
                        </span>
                        <span style={{
                          fontSize: 14,
                          fontWeight: 600,
              ...
              {totalAdjustment !== 0 && (
                <div style={{
                  ...
                  ℹ️ Your payment has been {totalAdjustment > 0 ? 'increased' : 'decreased'} by {formatCurrency(Math.abs(totalAdjustment))} due to {totalAdjustment > 0 ? 'overages' : 'shortages'}
                </div>
              )}
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {orderDiscrepancies.discrepancies.map((disc) => {
              ...
              const discTypeLabel = isOverage ? '📦 OVERAGE' : '⚠️ SHORTAGE';
