 ...
 * Allows kitchen owner to manage their staff members
...
export default function KitchenStaffManagement() {
  ...
  const fetchStaff = async () => {
    try {
      setLoading(true);
      // Get staff for this kitchen
      const data = await staffService.getStaff({ kitchen_id: user.kitchen_id });
      setStaff(data);
    } catch (err) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: '#6b7280', marginTop: 4, fontSize: 14 }}>
            Kitchen staff can accept and dispatch orders
          </p>
        </div>
        <button
